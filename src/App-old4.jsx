...
export default function App() {
  ...
  const handleCanvasMouseDown = (e) => {
    if (!commentMode || wasDragging || justDismissed) return;
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        setTimeout(() => setJustDismissed(false), 100);
      }
    };
  ...
  return (
    <>
      ...
      >
        {/* Embedded prototype */}
        <iframe
          src="https://cpq-mobile-v1.vercel.app/mobile-selection"
          title="Prototype"
          className="w-full h-[1000px] border-2 border-gray-200"
        />
        ...
        {inputPos && commentMode && (
          <div
            ref={inputRef}
            className="absolute bg-yellow-200 border border-red-500 rounded shadow p-2 z-50"
            style={{ top: `${inputPos.y}px`, left: `${inputPos.x}px` }}
          >
            <input
              autoFocus
